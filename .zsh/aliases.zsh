...
#
# When managing any dotfiles, use `dotfiles`. When managing the repository itself, go to the folder and use normal `git`.
# Both will list a number of "deleted" files, but this is due to the different working trees. Make sure to be careful
# so only relevant changes are committed.
